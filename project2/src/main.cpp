...
/*
    ...
    Responsibilities:
      - Display the main menu and the two sub-menus (interactive / performance).
      - Delegate all queue operations to InteractiveMenu<QueueImpl, T>.
      - Delegate all benchmarking to PerformanceTester<QueueImpl, T>.
      - When testing both implementations, collect config once and share it
        between the two testers so results land in the same output directory
        and are trivially comparable by filename.

    No free functions exist outside this class — all logic is encapsulated here
    or in the respective template helpers.
*/
class MainMenu {
private:
    // ── Console helpers ───────────────────────────────────────────────────────
