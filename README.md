...
* **Main features:** Adding elements (at the beginning, at the end, or at a specific index), removing elements, searching for values, and iterating through the list.
* **Educational goal:** Understanding dynamic memory allocation and operations on pointers/references.

### 2. Project: Queues
This directory contains the implementation of a queue (FIFO - First In, First Out).
...
The final project focuses on implementing a dictionary (map) structure, which allows storing data as key-value pairs (e.g., using a hash table or a BST tree).
